...
pub fn ternary_search_rec<T: Ord>(
    ...
    if end >= start {
        ...
        return match target.cmp(&list[mid1]) {
            Ordering::Less => ternary_search_rec(target, list, start, mid1 - 1),
            Ordering::Equal => Some(mid1),
            Ordering::Greater => match target.cmp(&list[mid2]) {
                Ordering::Greater => ternary_search_rec(target, list, mid2 + 1, end),
                Ordering::Equal => Some(mid2),
                Ordering::Less => ternary_search_rec(target, list, mid1 + 1, mid2 - 1),
            },
        }
    }
